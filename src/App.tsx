...
export default function App() {
  ...
  return (
    <div className="h-screen bg-[#0A0A0B] text-[#E0E0E0] font-sans flex flex-col overflow-hidden selection:bg-[#00F0FF]/30 selection:text-[#00F0FF]">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-4 md:p-6 flex flex-col min-h-0">
        {activeScreen === 'status' ? (
          <motion.div 
            ...
            className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-y-auto pb-8 min-h-0"
          >
      ...
      <footer className="mt-auto shrink-0 h-10 bg-[#00F0FF]/5 border-t border-[#00F0FF]/20 px-4 md:px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="text-[9px] text-red-500 hover:text-red-400 hover:underline uppercase tracking-widest"
          >
            Reset System
          </button>
        </div>
        ...
        {showResetWarning && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div 
              ...
              className="w-full max-w-md bg-[#0A0A0B] border border-red-500/50 shadow-[0_0_30px_rgba(239,68,68,0.2)] overflow-hidden flex flex-col"
            >
              <div className="bg-red-500/10 border-b border-red-500/30 p-4 flex items-center gap-3">
                <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></div>
                <h2 className="text-red-500 font-bold tracking-widest uppercase text-sm">System Warning</h2>
              </div>
              <div className="p-6 space-y-4 text-center">
                <p className="text-[#E0E0E0] text-sm uppercase tracking-wide leading-relaxed">
                  Player, are you sure you want to reset the system?
                </p>
                <p className="text-[#555] text-xs uppercase tracking-widest">
                  All progress, stats, and quests will be permanently erased.
                </p>
              </div>
              <div className="flex gap-4 p-4 border-t border-[#222] bg-[#121212]">
                <button 
                  onClick={() => setShowResetWarning(false)}
                  className="flex-1 py-3 text-xs uppercase tracking-widest font-bold border border-[#333] text-[#555] hover:text-[#E0E0E0] hover:border-[#555] transition-colors"
                >
                  Cancel
                </button>
                <button 
                  ...
                  className="flex-1 py-3 text-xs uppercase tracking-widest font-black bg-red-500/10 text-red-500 border border-red-500 hover:bg-red-500 hover:text-black transition-colors"
                >
                  Confirm Reset
                </button>
              </div>
